...
def tracking(model, opt, classes):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/yolov3-origin.cfg', help='cfg file path')
    parser.add_argument('--weights', type=str, default='weights/yolov3-80.weights', help='path to weights file')
    ...
    parser.add_argument('--data_cfg', type=str, default='cfg/coco-origin.data', help='coco.data file path')
    ...
    # Clean work space
    shutil.rmtree(opt.img_root)
    shutil.rmtree(opt.output_root)
    os.mkdir(opt.img_root)
    os.mkdir(opt.output_root)

    with torch.no_grad():
        yolo_model = load_model(opt)
        tracking(yolo_model, opt, classes)
